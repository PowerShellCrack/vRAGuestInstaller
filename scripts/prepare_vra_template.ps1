...
function extractZip($zipfile,$destination)
{
    ...
    # Windows Core requires Powershell 5 or up.
...
if (!$NoWindowsCheck)
{
    ...
    {
        Write-Feedback-Error "This script must exit due to unsupported operating system. $os is not supported please execute against Windows 2012, 2012 R2, 2016 or Windows 7, 8 and 10 only!"
    }
}
else
{
...
$bootstrapFile="https://" + $ApplianceHost + ":" + $AppliancePort + "/software/download/vmware-vra-software-agent-bootstrap-windows_7.5.0.0.zip"
...
else
{
    ...
    #
    # Capture stderr and stdout while running install.bat eg. errors such as bad credentials caught here.
    #
    $bootstrapInstallCommand = (Start-Process "$prepareFolder\install.bat" "$argumentList" -NoNewWindow -Wait -PassThru)
    if ($bootstrapInstallCommand.ExitCode -ne 0) {
        Write-Feedback-Error "Abort: Failed to install agent bootstrap service. Please correct this error and retry. You should reset between re-runs by running appd_bootstrap_removal script and emptying C:\opt and C:\VRMGuestAgent folders."
    }
